      ...
      >
        <div className="mb-6 flex items-center justify-between px-2">
          <Link href="/" className="flex min-w-0 items-center gap-2.5">
            <div className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-[6px] bg-foreground text-[11px] font-bold text-background">
              EA
            </div>
            {!collapsed && (
        ...
        <nav className="flex flex-col gap-0.5">
          {NAV_ITEMS.map((item) => {
            ...
            const badge =
              item.href === "/decisions" && decisionCount > 0 ? decisionCount : null;
            return (
              <Link
                key={item.href}
                href={item.href}
                onClick={onCloseMobile}
                className={cn(
                  "group relative flex items-center gap-2.5 rounded-md py-[7px] pl-3 pr-2.5 text-[length:var(--text-sm)] font-medium transition-colors",
                  active
              ...
              >
                <span
                  className={cn(
                    "absolute left-[-10px] top-[20%] bottom-[20%] w-[2px] rounded-full bg-foreground opacity-0 transition-opacity",
                    active && "opacity-100"
                  )}
                />
                ...
                {!collapsed && badge !== null && (
                  <span
                    className={cn(
                      "ml-auto font-mono text-[length:var(--text-2xs)]",
